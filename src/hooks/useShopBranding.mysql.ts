...
function mapShopBranding(data: any): ShopBranding {
  if (!data) return data;
  return {
    id: data.id,
    seller_id: data.sellerId || data.seller_id,
    theme_preset: data.themePreset || data.theme_preset || 'default',
    ...
    text_color: data.textColor || data.text_color,
    font_family: data.fontFamily || data.font_family || 'Inter',
    banner_type: data.bannerType || data.banner_type || 'image',
    banner_url: data.bannerUrl || data.banner_url,
    banner_video_url: data.bannerVideoUrl || data.banner_video_url,
    layout_style: data.layoutStyle || data.layout_style || 'grid',
    show_seller_avatar: data.showSellerAvatar ?? data.show_seller_avatar ?? true,
    show_stats: data.showStats ?? data.show_stats ?? true,
    show_badges: data.showBadges ?? data.show_badges ?? true,
...
export const useShopBranding = (sellerId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('shop_branding')
        .select('*')
        .eq('seller_id', sellerId)
        .single();

      if (error && !error.message?.includes('not found')) throw error;
      return data ? mapShopBranding(data) : null;
    },
    enabled: !!sellerId
  });
};
...
export const useUpdateShopBanner = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['shop-branding', data.seller_id] });
