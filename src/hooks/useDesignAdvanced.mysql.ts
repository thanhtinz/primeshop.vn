// MySQL version - useDesignAdvanced
...
export const useDesignLicenseTypes = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_license_types')
        .select('*')
        .eq('is_active', true)
        .order('sort_order');
...
export const useDesignMilestones = (orderId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_order_milestones')
        .select('*')
        .eq('order_id', orderId)
        .order('sort_order');
...
export const useDesignServiceTemplates = (serviceId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_service_templates')
        .select('*')
      ...
      return (data || []).map((item: any) => ({
        ...item,
        form_fields: (item.form_fields || item.formFields || []) as FormField[]
...
export const useDeleteDesignTemplate = () => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await apiClient.from('design_service_templates')
        .delete()
        .eq('id', id);
...
export const useDesignTeamMembers = (sellerId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data: members, error } = await apiClient.from('design_team_members')
        .select('*')
        .eq('seller_id', sellerId)
        .eq('is_active', true)
        .order('role');
      ...
      const memberIds = members.map((m: any) => m.member_user_id || m.memberUserId);
      ...
      return members.map((member: any) => ({
        ...member,
        member: profileMap.get(member.member_user_id || member.memberUserId) || null,
      })) as DesignTeamMember[];
    },
    enabled: !!sellerId,
  });
...
export const useAddDesignTeamMember = () => {
  ...
  return useMutation({
    mutationFn: async (member: { seller_id: string; member_user_id: string; role: string; can_view_orders?: boolean; can_manage_orders?: boolean; can_chat_buyers?: boolean; can_manage_finances?: boolean }) => {
      const { data, error } = await apiClient.from('design_team_members')
        .insert(member)
        .select()
        .single();
...
export const useDesignRevisionPackages = (orderId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_revision_packages')
        .select('*')
        .eq('order_id', orderId)
...
export const usePurchaseRevisionPackage = () => {
  ...
  return useMutation({
    mutationFn: async ({ orderId, quantity, pricePerRevision }: { orderId: string; quantity: number; pricePerRevision: number }) => {
      const { data, error } = await apiClient.post('/design/revision-package/purchase', {
        orderId,
        quantity,
        pricePerRevision,
      });
      ...
      if (!data.success) {
        throw new Error(data.error || 'Không thể mua gói chỉnh sửa');
      }
...
export const useDesignSellerRewards = (sellerId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_seller_rewards')
        .select('*')
        .eq('seller_id', sellerId)
...
export const useDesignSellerPenalties = (sellerId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_seller_penalties')
        .select('*')
        .eq('seller_id', sellerId)
...
export const useDesignEmailLogs = (orderId?: string, ticketId?: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      let query = apiClient.from('design_email_logs').select('*');
...
export const useCreateAbuseReport = () => {
  ...
  return useMutation({
    mutationFn: async (report: { reported_user_id: string; order_id?: string; ticket_id?: string; abuse_type: string; description?: string; evidence?: any }) => {
      const { data, error } = await apiClient.from('design_abuse_reports')
        .insert({
          ...report,
          reporter_id: user?.id,
        })
...
export const useDesignPlatformFees = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_platform_fees')
        .select('*')
        .eq('is_active', true);
...
export const useDesignSellerMatchProfile = (sellerId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_seller_match_profiles')
        .select('*')
        .eq('seller_id', sellerId)
        .maybeSingle();
...
export const useUpdateDesignMatchProfile = () => {
  ...
  return useMutation({
    mutationFn: async ({ sellerId, ...updates }: { sellerId: string; is_available?: boolean; max_concurrent_orders?: number; categories?: string[]; specialties?: string[] }) => {
      // Check if exists
      const { data: existing } = await apiClient.from('design_seller_match_profiles')
        .select('id')
        .eq('seller_id', sellerId)
        .maybeSingle();
...
export const useDesignSellerDailyStats = (sellerId: string | undefined, days: number = 30) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await apiClient.from('design_seller_daily_stats')
        .select('*')
        .eq('seller_id', sellerId)
...
export const useDesignNDA = (orderId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_nda_agreements')
        .select('*')
        .eq('order_id', orderId)
        .maybeSingle();
...
export const useAutoMatchSellers = (categoryId: string | undefined, budget: number) => {
  return useQuery({
    ...
    queryFn: async () => {
      // Call auto-match API
      const { data, error } = await apiClient.post('/design/auto-match', { categoryId, budget });
      if (error) throw error;
      return data?.sellers || [];
    },
...
export const useDesignAuditLogs = (orderId?: string, ticketId?: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      let query = apiClient.from('design_audit_logs').select('*');
...
export const useDesignFileVersions = (orderId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_file_versions')
        .select('*')
        .eq('order_id', orderId)
...
export const useCreateFileVersion = () => {
  ...
  return useMutation({
    mutationFn: async (version: Omit<DesignFileVersion, 'id' | 'created_at' | 'version_number'>) => {
      const { data: existingVersions } = await apiClient.from('design_file_versions')
        .select('version_number')
        .eq('order_id', version.order_id)
        .order('version_number', { ascending: false })
        .limit(1);

      const nextVersion = (existingVersions?.[0]?.version_number || existingVersions?.[0]?.versionNumber || 0) + 1;
...
export const useDesignReviewCriteria = (serviceId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_review_criteria')
        .select('*')
        .eq('service_id', serviceId)
        .order('created_at', { ascending: false });
...
export const useDesignSellerNotes = (ticketId: string | undefined, sellerId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_seller_notes')
        .select('*')
...
export const useSaveSellerNote = () => {
  ...
  return useMutation({
    mutationFn: async ({ ticketId, sellerId, note }: { ticketId: string; sellerId: string; note: string }) => {
      const { data: existing } = await apiClient.from('design_seller_notes')
        .select('id')
        .eq('ticket_id', ticketId)
        .eq('seller_id', sellerId)
        .maybeSingle();
...
export const useDesignActivityLogs = (orderId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_activity_logs')
        .select('*')
        .eq('order_id', orderId)
        .order('created_at', { ascending: false });
...
export const useCreateReport = () => {
  ...
  return useMutation({
    mutationFn: async (report: Omit<DesignReport, 'id' | 'created_at' | 'updated_at' | 'reporter_id' | 'status' | 'admin_notes' | 'resolved_at' | 'resolved_by'>) => {
      const { data, error } = await apiClient.from('design_reports')
...
export const useDesignQuickActions = (userType: 'buyer' | 'seller') => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await apiClient.from('design_quick_actions')
        .select('*')
        .eq(column, true)
        .eq('is_active', true)
        .order('sort_order');
...
export const useBlockedKeywords = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_blocked_keywords')
        .select('*')
        .eq('is_active', true);
...
export const useUpdateSellerAvailability = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { data, error } = await apiClient.from('sellers')
        .update({
...
export const useUpdateTicketPriority = () => {
  ...
  return useMutation({
    mutationFn: async ({ ticketId, priority, isVip }: { ticketId: string; priority: number; isVip?: boolean }) => {
      const { data, error } = await apiClient.from('design_tickets')
...
export const useDesignTicketCollaborators = (ticketId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_ticket_collaborators')
        .select('*')
        .eq('ticket_id', ticketId)
        .eq('is_active', true)
...
export const useDesignTicketInternalNotes = (ticketId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.from('design_ticket_internal_notes')
        .select('*')
        .eq('ticket_id', ticketId)
